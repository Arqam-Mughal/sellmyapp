          ...
          <form action="" method="post" name="platformForm" id="platformForm">
            ...
            <div class="card-body">
              <div class="form-group">
                <label>Name</label>
                <input type="text" name="name" id="name" value="{{ $platform->name }}" class="form-control">
                <p></p>
              </div>
              
              <div class="form-group">
                <label>Slug</label>
                <input type="text" readonly name="slug" id="slug" class="form-control" value="{{ $platform->slug }}">
                <p></p>
              </div>
              ...
              <div class="form-group mb-0">
                ...
                <select name="status" id="status" class="form-control">
                  <option {{ ($platform->status == 1) ? 'selected' : '' }} value="1">Active</option>
                  <option {{ ($platform->status == 0) ? 'selected' : '' }} value="0">Block</option>
                </select>
              </div>
...
<script>
  $('#platformForm').submit(function(e) {
    ...
    $.ajax({
      url: '{{ route("admin.platform.update", $platform->id) }}',
      type: 'put',
